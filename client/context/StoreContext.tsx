...
const loadStateFromStorage = (): Partial<State> => {
  try {
    const savedState = localStorage.getItem('musemart-store');
    if (savedState) {
...
const saveStateToStorage = (state: State) => {
  try {
    ...
    localStorage.setItem('musemart-store', JSON.stringify(stateToSave));
  } catch (error) {
    console.error('Failed to save state to localStorage:', error);
  }
};

// Обновите начальное состояние с загрузкой из localStorage
const savedState = loadStateFromStorage();
const initialState: State = {
  products: seedProducts,
  users: savedState.users || seedUsers,
  orders: savedState.orders || seedOrders,
